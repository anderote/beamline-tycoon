...
*Silent failures if you touch this:* without migration, a drained reservoir refills for free on any topology edit; without the cap, joining two independently seeded loops produces `N × RESERVOIR_MAX_L`, `refillCost()` computes a negative `missing` and returns `null`, and the Refill button silently vanishes; without the grace window, delete-and-redraw is a free refill for two clicks.
