...
`registry.utilityLineHeight` supplies the per-utility default. Fabricated vacuum, cryogenic, and RF services persist `routeHeightMeters`; `route-elevation.js` selects the first vertically clear lane at or above the starting connector, while deliberate taps inherit their trunk's lane. The input tool follows the start anchor and resolved preview height. Even half a metre of mismatch is a 15–25 px iso offset, so treating elevation as cosmetic makes drawing land up-screen of the cursor. `network-discovery.js` also checks lane height before turning endpoint-on-line plan contact into a physical join.
