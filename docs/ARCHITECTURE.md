...
### `beam_physics/` + bridge
A pure-Python package run **client-side under Pyodide** (`src/beamline/physics.js`), and also directly under pytest (`test/test_*.py`, run by `scripts/run-tests.mjs`). `gameplay.py` is the single entry point (`compute_beam_for_game`); `lattice.propagate` runs the module stack in `modules/`; `machines.py` maps a beamline type to a module configuration; `srf.py` owns cavity specs and the gradient model. The JS side passes JSON through `pyodide.globals`, never string interpolation (`physics.js:97-99`).
...
Three lists, all in `Game.js`: `UNDO_PRESERVED_FIELDS` (`:78-84`) — clock, research, objectives, staff, log, and `savedDesigns` (which lives outside any gesture); `BEAMSTATE_PRESERVED_FIELDS` (`:92-95`) — per-entry accumulators; and the resource ledger (`_syncResourceLedger`, `:719`), which lets undo refund a build's cost without reclaiming the upkeep paid while it stood. The RNG stream is **not** reseeded on undo (`:4548-4554`).
...
**U7. Utility lines run at `UTILITY_LINE_Y = 0.5` and that is geometric, not cosmetic.**
`line-geometry.js:14-18`: the input tool picks against a plane at this height. Half a metre of elevation is 15–25 px of iso offset, so a mismatch makes the drawing land up-screen of the cursor.
...
`network-discovery.js:24-32`; `SolveRunner._reconcilePersistentState` (`solve-runner.js:173-278`) migrates persistent state across the re-hash by `__portKeys` overlap, splits it proportionally on a cut, sums it on a join, and **caps it at `persistentStateDefaults`** (which is itself a full reservoir, `:240-257`). Orphans are held for `ORPHAN_GRACE_PASSES = 300` (`:34`) before pruning.
Cooling water is the dynamic-cap exception: `coolingWater.boundPersistentState` caps inventory at the sum of connected `storageCapacityL` ports, because adding a second physical tank must add real capacity. Fixed-cap utilities (currently cryo) still use `persistentStateDefaults` as the ceiling.
*Silent failures if you touch this:* without migration, a drained reservoir refills for free on any topology edit; without the appropriate fixed or dynamic cap, joining independently seeded loops can leave inventory above physical storage, make `refillCost()` compute a negative `missing`, and silently remove the Refill button; without the grace window, delete-and-redraw is a free refill for two clicks.
...
**B2. `physics.js:PY_MODULES` must list every Python file the package imports — and it currently does not.**
`physics.js:13-32` writes each listed file into Pyodide's virtual FS and then imports `beam_physics.gameplay`. `gameplay.py:12` does `from beam_physics import srf`, and `machines.py:30` does `from beam_physics.modules.beam_gas import BeamGasModule`. **Neither `beam_physics/srf.py` nor `beam_physics/modules/beam_gas.py` appears in `PY_MODULES`.** Nothing else fetches them (verified by grep across `src/`, `index.html`, `scripts/`, `vite.config.js`). Nothing tests this — pytest imports the real filesystem package, so the suite passes.
*Silent failure:* `init()` throws a `ModuleNotFoundError`, is caught at `physics.js:75-77`, and every beamline permanently falls back to `Game._fallbackStatsForBeamline` (`Game.js:3325`). The game runs; the physics is just not the physics. **Treat this as a live bug, not documented behaviour — verify in a browser before changing anything nearby.**
