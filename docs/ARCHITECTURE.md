...
`registry.utilityLineHeight` supplies the per-utility default. Fabricated vacuum, cryogenic, and RF services persist `routeHeightMeters`; `route-elevation.js` selects the first vertically clear lane at or above the preferred route height, while deliberate taps inherit their trunk's lane. Vacuum and RF prefer the higher connector; cryogenic transfer prefers its low service rack and uses an outside-the-cryostat endpoint drop for elevated bayonets. The input tool follows the resolved preview height. Even half a metre of mismatch is a 15–25 px iso offset, so treating elevation as cosmetic makes drawing land up-screen of the cursor. `network-discovery.js` also checks lane height before turning endpoint-on-line plan contact into a physical join.
