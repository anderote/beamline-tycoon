...
interaction, or visual integration.
...
Use an unused ephemeral port and launch Vite directly with `--strictPort`, as
documented in `AGENTS.md`. Track and terminate that exact process, then confirm
the selected port has no listener. Never stop or replace port 8000.
