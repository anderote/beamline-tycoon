...
console.log('\n=== fit rule ===\n');
{
  ...
  assert(g.placeWall(21, 20, 'e', 'cubicleWall'), 'setup: cubicleWall (wallHeight 8) at 21,20,e');
  const fundingBefore = g.state.resources.funding;
  assert(!g.placeWindow(21, 20, 'e', 'officeWindow'),
    'sillHeight(5)+openingHeight(6)+1 = 12 does not fit cubicleWall(8): no-op');
...
console.log('\n=== a shorter replacement wall evicts a window it cannot hold ===\n');
{
  ...
  assert(g.placeWall(60, 7, 'e', 'cubicleWall'), 'replacing with cubicleWall (wallHeight 8) succeeds');
