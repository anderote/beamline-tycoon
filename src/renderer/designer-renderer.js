...
function _missionBandForPlot(type, plotType) {
  if (!type) return null;
  if (plotType === 'energy-dispersion') return type.spec?.energyGeV || null;
  if (plotType === 'current-loss') return type.spec?.currentMA || null;
  return null;
}

// Keep the mission band on the same axes as the plotted beam, including when
// the proposal is still outside the target. Otherwise the shaded band would be
...
BeamlineDesigner.prototype._renderPlots = function() {
  ...
  const missionType = getBeamlineType(this._designerBeamlineTypeId?.());
  ...
  panels.forEach((panel) => {
    ...
    if (!solid) {
      ...
      const targetBand = _missionBandForPlot(missionType, plotType);
      const yDomain = _includeMissionBand(ProbePlots.unionYDomain(
      ...
      if (ghost) {
        ProbePlots.draw(off, plotType, ghost, pins, 0, xRange, yScale,
          { yDomain, targetBand, ghost: true });
      }
      ProbePlots.draw(off, plotType, solid, pins, 0, xRange, yScale,
        { yDomain, targetBand, noClear: !!ghost });
    }
