...
// Adjacent segments merge into continuous runs with shared supports.
//
...
import {
  ...
  junctionType, junctionRotation,
} from '../data/carrier-rack.js';

const TILE_W = 2.0;
const SEG_LEN = RACK_SIZE * TILE_W;
const SUPPORT_INTERVAL = 3; // supports every N tiles on straight runs
...
export class RackBuilder {
  ...
  build(rackData, parentGroup) {
    ...
    for (const seg of rackData) {
      ...
      const type = junctionType(seg.neighbors);
      const needsLegs = this._needsSupport(type, seg);

      this._buildSegment(wrapper, needsLegs);
  ...
  _needsSupport(type, seg) {
    if (type !== 'straight') return true;

    // On straight runs, place supports at regular intervals.
    // Use position modulo to get deterministic spacing regardless of build order.
    const { north, south, east, west } = seg.neighbors;
    const isNS = north && south;
    const coord = isNS ? seg.row : seg.col;
    return coord % SUPPORT_INTERVAL === 0;
  }

  _buildSegment(group, needsLegs) {
    ...
    if (needsLegs) {
      // Two support legs (one on each side of the tray)
