...
export class ThreeRenderer {
  ...
  async init() {
    ...
    // rewrite.
    const fixtureShadowSlots = this._rendererBackend.mode === 'modern'
      ? MAX_FIXTURE_SHADOWS
      : fixtureShadowTopologyLimit(this._rendererBackend.capabilities.maxTextures);
    this._lightRig = new LightRig(this.scene, {
      enabled: glowStored !== '0',
      modernRenderer: this._rendererBackend.mode === 'modern',
      fixtureLightCount: this._rendererBackend.mode === 'modern'
        ? MAX_FIXTURE_LIGHTS
        : fixtureShadowSlots,
      activeFixtureLightCount: this._rendererBackend.mode === 'modern'
        ? this._lightingQuality.fixtureLightCount
        : fixtureShadowSlots,
  ...
  getLightingStats() {
    return {
      rendererMode: this._rendererBackend?.mode || 'unknown',
      requestedRendererMode: this._rendererBackend?.requestedMode || 'default',
  ...
  _setShadowMapSize(shadow, mapSize) {
    ...
    // WebGPURenderer's ShadowNode owns this render target. Disposing it from
    // application code leaves the node holding a destroyed depth texture and
    // the next queued submit fails validation. Changing mapSize is enough;
    // ShadowNode resizes its target immediately before the next shadow pass.
    if (shadow.map && this._rendererBackend?.mode !== 'modern') {
      shadow.map.dispose();
      shadow.map = null;
    }
   ...
   * uniform-density bodies. Call undoLastPhysicsIncident() to snap the whole
  ...
  _addPlaceableGhostMeshes(hover, valid, reason = null) {
    ...
    // Point toward the model's meaningful operating face. Most devices follow
    // the gameplay direction directly; sources and explicitly authored +Z
    // service faces reverse it. Decorations have no operator-facing side.
    if (placeable.kind !== 'decoration') {
      const arrowDir = placementFacingArrowDir(placeable, hover.dir || 0);
  ...
  _renderGridAroundCursor(col, row) {
    ...
    // --- Per-tile dotted borders ---
    const majorBuffers = { positions: [], colors: [] };
    const addMajorSegment = (start, end) => appendPlacementGridDots(
      majorBuffers,
      start,
      end,
      {
        spacing: PLACEMENT_GRID_STYLE.majorDotSpacingWorld,
        dotLength: PLACEMENT_GRID_STYLE.majorDotLengthWorld,
        startAlpha: alphaAt('major', start.x, start.z),
        endAlpha: alphaAt('major', end.x, end.z),
      },
    );
    ...
    majorLines.userData.dotSpacingWorld = PLACEMENT_GRID_STYLE.majorDotSpacingWorld;
    majorLines.userData.dotLengthWorld = PLACEMENT_GRID_STYLE.majorDotLengthWorld;
    ...
    const subVerts = [];
    const subColors = [];
    const subBuffers = { positions: subVerts, colors: subColors };
    const addSubSegment = (ax, ay, az, bx, by, bz, patternOffsetWorld) => {
      appendPlacementGridDots(
        subBuffers,
        { x: ax, y: ay, z: az },
        { x: bx, y: by, z: bz },
        {
          spacing: PLACEMENT_GRID_STYLE.subgridDotSpacingWorld,
          dotLength: PLACEMENT_GRID_STYLE.subgridDotLengthWorld,
          patternOffsetWorld,
          startAlpha: alphaAt('subgrid', ax, az),
          endAlpha: alphaAt('subgrid', bx, bz),
        },
      );
    };
    for (let dr = -subRadius; dr <= subRadius; dr++) {
      for (let dc = -subRadius; dc <= subRadius; dc++) {
        ...
        for (let sub = 1; sub <= 3; sub++) {
          ...
          addSubSegment(xa, yW, z, xMid, yMid, z, 0);
          addSubSegment(xMid, yMid, z, xb, yE, z, xMid - xa);
        }
        ...
        for (let sub = 1; sub <= 3; sub++) {
          ...
          addSubSegment(x, yN, za, x, yMid, zMid, 0);
          addSubSegment(x, yMid, zMid, x, yS, zb, zMid - za);
        }
      }
    }
    ...
    subLines.userData.dotSpacingWorld = PLACEMENT_GRID_STYLE.subgridDotSpacingWorld;
    subLines.userData.dotLengthWorld = PLACEMENT_GRID_STYLE.subgridDotLengthWorld;
  ...
  _animate() {
    ...
    const cameraMoving = this._viewRotating || this._snapping || this._focusing
      || performance.now() < this._cameraMotionUntilMs;
    this._updateZoneLabelFacing();
    if (this._updateAnchoredWindows) this._updateAnchoredWindows();
    this._updateSunCycle(cameraMoving);
    ...
    if (this._lightRig) {
      ...
      this._lightRig.update(
        ...
        { freezeAssignment: cameraMoving, deferShadows: cameraMoving },
      );
    }
  ...
  _updateSunCycle(deferShadows = false) {
    ...
    const sunUpdates = this._sunShadowScheduler?.step({
      activeCount: 1,
      enabled: !deferShadows
        && this.renderer.shadowMap.enabled && grade.sunIntensity > 0.02,
      dtMs: dt * 1000,
      assignmentKeys: ['sun'],
    }) || [];
