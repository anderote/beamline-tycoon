...
export function utilityPerformanceSample(flow, tick) {
  ...
  return Object.freeze({
    ...
    roughingCapacity: finiteOrNull(flow?.stageCapacities?.rough?.powered),
    highVacCapacity: finiteOrNull(flow?.stageCapacities?.high?.backed),
    uhvCapacity: finiteOrNull(flow?.stageCapacities?.uhv?.powered),
    effectivePumpSpeed: finiteOrNull(flow?.effectivePumpSpeed),
    evacuatedVolumeL: finiteOrNull(flow?.volumeL),
    beamPipeVolumeL: finiteOrNull(flow?.volumeBreakdown?.beamPipeL),
    servicePipeVolumeL: finiteOrNull(flow?.volumeBreakdown?.servicePipeL),
    componentChamberVolumeL: finiteOrNull(flow?.volumeBreakdown?.componentChambersL),
    connectedNodeCount: finiteOrNull(flow?.connectedNodeCount),
    connectedLinkCount: finiteOrNull(flow?.connectedLinkCount),
    deltaT: finiteOrNull(flow?.deltaT),
    reservoirVolumeL: finiteOrNull(flow?.reservoirVolumeL),
    storageCapacityL: finiteOrNull(flow?.storageCapacityL),
    evaporationL: finiteOrNull(flow?.evaporationL),
    suppliedWaterL: finiteOrNull(flow?.suppliedWaterL),
    tempK: finiteOrNull(flow?.tempK),
    designTempK: finiteOrNull(flow?.designTempK),
    staticLoad: finiteOrNull(flow?.staticLoad),
    dynamicLoad: finiteOrNull(flow?.dynamicLoad),
    boiloffL: finiteOrNull(flow?.boiloffL),
    recoveredL: finiteOrNull(flow?.recoveredL),
    netLheLossL: finiteOrNull(flow?.netLheLossL),
    meanDuty: finiteOrNull(flow?.meanDuty),
    vswr: finiteOrNull(flow?.vswr),
    branchReflectionFraction: finiteOrNull(flow?.branchReflectionFraction),
